clear
load('100 designs for the shaft example.mat');% 100 design points given by ShaftDesignpoints.m
Nx_e=10;[x_e0,w_noise0]=lgwt(Nx_e,-1,1);
x_e=0.5*x_e0+0.5;w_noise=0.5*w_noise0;
...
% compute the true EQL at x_c grid.
qgrid=zeros(nx,1);
parfor i=1:nx
    qgrid(i)=TrueEQL(x_c(i),xcgrid,xegrid,yhgrid,ymgrid,w,x_e);
end
tic

% choose one design
choose_d=12
xl=reshape(xlall(choose_d,:),nd,[])';
...
% the estimated optimal control-factor setting and the true EQL at the estimated optimal control-factor setting given by the ASLLGP, ALL and AGPL models.
...
minALLEQL=TrueEQL(xcminALL,xcgrid,xegrid,yhgrid,ymgrid,w,x_e);
minASLLEQL=TrueEQL(xcminASLL,xcgrid,xegrid,yhgrid,ymgrid,w,x_e);
minAGPLEQL=TrueEQL(xcminAGPL,xcgrid,xegrid,yhgrid,ymgrid,w,x_e);
minEQLtrue=[minq,minASLLEQL,minALLEQL,minAGPLEQL]
%% Figure 3 the plots of the posterior mean (point predictor), and lower and upper 95% credible limits for the EQL given by the ASLLGP, ALL and AGPL models versus the control factor
...
ylabel('EQL','FontSize',ft,'FontWeight','bold');
ylim([ylbgpl,yubgpl]);set(gca,'yTick',[ygpl]);set(gca,'YTickLabel',{'-21','-11','0','12','26','41','58','78'});
set(gca,'fontsize',ft,'fontweight','bold');
...
set(gca,'fontsize',ft,'fontweight','bold');
%%
function q=TrueEQL(x_c,xcgrid,xegrid,yhgrid,ymgrid,w,x_e)
%Function for computing the true EQL
Nx_e=size(x_e,1);
ystemp=zeros(Nx_e,1);ymtemp=zeros(Nx_e,1);
losstemps=zeros(Nx_e,1);losstemp=zeros(Nx_e,1);
losstempm=zeros(Nx_e,1);
for k=1:Nx_e
    ystemp(k)=interp2(xcgrid,xegrid,yhgrid,x_c,x_e(k));
    ymtemp(k)=interp1(xcgrid(1,:),ymgrid(1,:),x_c);
    [losstemp(k),losstemps(k),losstempm(k)]=lossShaft(ystemp(k),ymtemp(k));
end
loss_1=w*losstemps;
q=loss_1+losstempm(1);% loss of mass does not depend on noise factors.

end
function logx=logscale(x)
%Function for the modified log scale to the vertical axes
C=2;
