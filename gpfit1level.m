...
%Outputs thetal:maximum integrated likelihood estimate of correlation parameters;fvall:-2*(log likelihood);betal:maximum likelihood estimate of the regression coefficients;tao2l:maximum likelihood estimate of variance parameter;irResl,irxl are several vectors and matrices to be used by gppredict to compute point predictions and prediction intervals
format long g
% global G x
G=yin;
x=xin;
...
parfor i=1:nstart
%     [par(i,:), fvaln(i)]=patternsearch(@omle,can_start(inds(i),:),[],[],[],[],lb*ones(1,d) ,ub*ones(1,d),[],options);
