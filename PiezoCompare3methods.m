...
L_dtest=zeros(Nx_e,nt);
L_mtest=zeros(Nx_e,nt);
for i=1:nt
    for j=1:Nx_e
        [temptest(j,i),L_dtest(j,i),L_mtest(j,i)]=lossPiezo(yd_test(j,i),ym_test(j,i));
    end
end
Ltest1=(w_noise*L_dtest)';Ltest2=L_mtest(1,:)';
qtest=Ltest2+Ltest1;
...
for ii=1:nx
    for jj=1:nx
losstempd=zeros(Nx_e,1);losstemp=zeros(Nx_e,1);
losstempm=zeros(Nx_e,1);       
        parfor kk=1:Nx_e
        [losstemp(kk),losstempd(kk),losstempm(kk)]=lossPiezo(yd_grid(ii,jj,kk),ym_grid(ii,jj,kk));
        end
   loss_1=w_noise*losstempd;
        [qgrid(ii,jj)]=loss_1+losstempm(1);
    end
end
...
parfor k=1:ndesign
    minALLEQL(k)=TrueEQL(xcminALL(k,:),points,w_noise);
    minASLLEQL(k)=TrueEQL(xcminASLL(k,:),points,w_noise);
    minAGPLEQL(k)=TrueEQL(xcminAGPL(k,:),points,w_noise);
end

minq=min(min(qgrid)),max(max(qgrid)),mean(mean(qgrid))
...
% Table 5: five performance measures in predicting EQL obtained with the ASLLGP, ALL, and AGPL models for the piezoelectric actuator example
mMARE=mean(MARE)*100
...
function q=TrueEQL(x_c,points,w_noise)
%Function for compute the true EQL
% global xcgrid xegrid yhgrid ymgrid w x_e
Nx_e=size(points,1);
Lent=100e-3+x_c(1)*100e-3;
Heit=0.6e-3+x_c(2)*0.6e-3;
d31t=1.5e-11+points(:,1)*1.5e-11;
vt=90+points(:,2)*20;
