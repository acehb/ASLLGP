function q=PiezoTrueEQL(x_c,x_e,w_noise,b)
...
%b:upper bound of the loss function(omit for unbounded loss function).
%Outputs q:true EQL
...
if(nargin==3)
    ...
    loss_1=w_noise*losstemps;q=loss_1+losstempm(1);
else
    for k=1:Nx_e
        [ydtemp(k),ymtemp(k)]=PiezoelectricActuator(Lent,Heit,d31t(k),vt(k),2);
        [losstemp(k),losstemps(k),losstempm(k)]=lossPiezo(ydtemp(k),ymtemp(k),b);
    end
    q=w_noise*losstemp;
end
