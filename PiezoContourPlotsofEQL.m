clear;
load('100 designs for the piezoelectric actuator example.mat');% 100 designs given by PiezoDesignPoints.m
load('400 test points and 2D grid points.mat');% 400 test points for the comparison of EQL prediction and simulation outputs for grid optimization given by PiezoTestPoints.m
...
tempgrid=zeros(nx,nx,Nx_e);qgrid=zeros(nx,nx);
for ii=1:nx
    for jj=1:nx     
        for kk=1:Nx_e
        [tempgrid(ii,jj,kk),~,~]=lossPiezo(yd_grid(ii,jj,kk),ym_grid(ii,jj,kk));
        end
    temp=reshape(tempgrid(ii,jj,:),[],1);
        [qgrid(ii,jj)]=w_noise*temp;
...
for i=1:n
     [Ll(i),Ll_s(i)]=lossPiezo(yld(i),ym(i));
end
for i=1:m
 [Lh(i),Lh_s(i)]=lossPiezo(yhd(i),ym(i));
end
%fit 3 models
...
%% Figure 5: Contour plots of the posterior mean (point predictor) for the EQL given by the ASLLGP, ALL, and AGPL models versus the control factor x_c.
range1=[1,2,4,6,8];rangeGPL=[0,1,2,4,6,8];
labs=21;lw=4;ls2=699;ls1=260;
...
ca1 = get(gca,'XTickLabel');set(gca,'XTickLabel',ca1,'fontsize',ftick,'fontweight','normal');
yticks([0:0.1:1]);
cay1 = get(gca,'YTickLabel');set(gca,'YTickLabel',cay1,'fontsize',ftick,'fontweight','normal');
...
xlabel('Control factor x_1','FontSize',labs,'FontWeight','normal');ylabel('Control factor x_2','FontSize',labs,'FontWeight','normal');
hall1.LabelSpacing=ls1;hall2.LabelSpacing=ls2;
xlim([0,1]);xticks([0:0.2:1]);
ca2 = get(gca,'XTickLabel');set(gca,'XTickLabel',ca2,'fontsize',ftick,'fontweight','normal');
yticks([0:0.1:1]);
cay2 = get(gca,'YTickLabel');set(gca,'YTickLabel',cay2,'fontsize',ftick,'fontweight','normal');
...
xlim([0,1]);xticks([0:0.2:1]);
ca3 = get(gca,'XTickLabel');set(gca,'XTickLabel',ca3,'fontsize',ftick,'fontweight','normal');
yticks([0:0.1:1]);
cay3= get(gca,'YTickLabel');set(gca,'YTickLabel',cay3,'fontsize',ftick,'fontweight','normal');
