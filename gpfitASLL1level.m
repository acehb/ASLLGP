...
%Outputs theta:maximum likelihood estimate of correlation parameters;fval:-2*(log likelihood);beta:maximum likelihood estimate of the regression coefficients;
%tao2:maximum likelihood estimate of variance parameter;irRes,irx are several vectors and matrices to be used by gppredict to compute point predictions and prediction intervals %transparl: maximum likelihood estimate of a_l and epsilon_l;
format long g
% global G x d Sign
...
can_start=[lb+(ub-lb)*X0(:,1:d),ube*X0(:,3)]; 
...
par=zeros(nstart,d+1); fvaln=zeros(nstart,1);
    parfor i=1:nstart
        if(i<=nstart/2)
            ...
            Sign=-1;
            [par(i,:), fvaln(i)]=patternsearch(@(par)omle(par,G, x, d, Sign),can_start(inds1(i)+nc_start/2,:),[],[],[],[],[lb*ones(1,d) ubound],[ub*ones(1,d) 10*ubound],[],options);
        end
...
% tao2=Res'*(RIRes)/(n-1);mle=ldetrx+(n-1)*log(max(tao2,0))+ldet1r1+2*sum(S);
