...
The *BarOutput.mat* file gives the maximum von Mises stress outputs of the HF and LF simulators (stored in the variable stress) and the mass of the small bar outputs of both simulators (stored in the variable mass) on the $11^2$ grid $\lbrace 0, 0.1,\ldots,1\rbrace^2$ of $(\textbf{x}_c,\textbf{x}_e)$ values.
...
The *Piezo True EQL on grid points*.*mat* file gives the true EQL at each $\textbf{x}_c\in\lbrace 0,0.005,\ldots,1\rbrace^2$, for both unbounded and bounded loss functions.
...
*gpfitASLL1level.m*: Function used to find the estimate of the vector of parameters $\textbf{Θ}_L$ for the proposed model in Section 4.

*gpfitASLL2level.m*: Function used to find the estimate of the vector of parameters $\textbf{Θ}$ for the proposed model in Section 4.
...
*PiezoTrueEQLonGridPoints.m*: Script used to compute the true EQL at each $\textbf{x}_c\in\lbrace 0,0.005,\ldots,1\rbrace^2$ for Example 2 and 3 (used to compute the data stored in the *Piezo True EQL on grid points*.*mat* file).
...
*BarInterp.m*: Interpolators for the mass of the small bar output (which is the same for both HF and LF simulators) on the grid $\lbrace 0, 0.1,\ldots,1\rbrace$  of $\textbf{x}_c$ values and the HF (fidelity=2) and LF (fidelity=1) maximum von Mises stress outputs on the grid $\lbrace 0, 0.1,\ldots,1\rbrace^2$ of $(\textbf{x}_c,\textbf{x}_e)$ values for Example 1.
